...
const COCKPIT_API_BASE_URL: &str = "https://api.newbeeapi.com";
const LEGACY_NEWBEE_API_BASE_URL: &str = "https://www.newbeeapi.com/v1";
...
fn is_cockpit_api_base_url(raw: Option<&str>) -> bool {
    ...
    [
        ...
        LEGACY_NEWBEE_API_BASE_URL,
        LEGACY_COCKPIT_API_BASE_URL,
    ]
    .iter()
    .filter_map(|expected| normalize_api_base_url_for_match(Some(expected)))
...
async fn fetch_new_api_profile_quota(account: &CodexAccount) -> Result<FetchQuotaResult, String> {
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|err| format!("Request Newbee API quota failed: {}", err))?;
    let status = response.status();
    let body = response
        .text()
        .await
        .map_err(|err| format!("Read Newbee API quota response failed: {}", err))?;
    if !status.is_success() {
        return Err(format!("Newbee API quota endpoint returned HTTP {}", status.as_u16()));
    }

    let root: serde_json::Value = serde_json::from_str(&body)
        .map_err(|err| format!("Parse Newbee API quota JSON failed: {}", err))?;
    if root.get("success").and_then(|item| item.as_bool()) == Some(false) {
        let message = root
            .get("message")
            .and_then(|item| item.as_str())
            .unwrap_or("Newbee API quota endpoint returned failure");
