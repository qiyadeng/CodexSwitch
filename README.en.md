...
- **API Service**: The local Codex API service is powered by the bundled CLIProxyAPI sidecar. Codex Switch handles account sync, config projection, status, and usage statistics while keeping the same Base URL, API keys, and user workflow.
...
- **macOS privacy permission prompts**: after you start Codex/agent from Codex Switch, if an agent-run shell command accesses protected folders such as Desktop, Documents, Downloads, or Photos, macOS may show the request as "Codex Switch would like to access...". This happens because those commands are child processes launched by Codex Switch, so macOS attributes the request to the host app; it does not by itself mean the Codex Switch main process is actively scanning those folders. Grant access only when you trust the current agent task and the commands it is going to run. If unsure, deny the prompt or run the project from a normal working directory first.
...
If Homebrew says the app already exists (e.g. `already an App at '/Applications/Codex Switch.app'`), remove the old app and install again:

```bash
rm -rf "/Applications/Codex Switch.app"
brew install --cask cockpit-tools
```
...
1.  **Command Line Fix** (Recommended):
    ...
    ```bash
    sudo xattr -rd com.apple.quarantine "/Applications/Codex Switch.app"
    ```
